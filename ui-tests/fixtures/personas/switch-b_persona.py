"""
Fixture persona for E2E tests: the second of the "Switch A" / "Switch B" pair
used to verify that switching the selected persona does not carry the previous
persona's model/settings selection over to the new one (see
tests/switch-personas.spec.ts, issue #62).

Structurally identical to `switch-a` apart from its name and the model IDs it
advertises. Echoes what a selection *applied*, so a reply proves whether the new
persona inherited a selection made against the previous one (it must not).
...
_MODELS = [
    ModelOption(id="b-one", name="B One"),
    ModelOption(id="b-two", name="B Two"),
]
_MODEL_CURRENT = "b-one"
...
class SwitchBPersona(BasePersona):
    """Test-only persona (B) that echoes the model/settings a selection applied."""
